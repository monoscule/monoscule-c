...
static void identify(struct ms_image img, struct ms_image *out, char *argv[]) {
  (void)out, (void)argv;
  printf("Portable Graymap, %ux%u (%u) pixels\n", img.w, img.h, img.w * img.h);
}

static void view(struct ms_image img, struct ms_image *out, char *argv[]) {
  (void)out, (void)argv;
...
static void resize(struct ms_image img, struct ms_image *out, char *argv[]) {
...
static void crop(struct ms_image img, struct ms_image *out, char *argv[]) {
  ...
  *out = ms_alloc(w, h);
  ms_crop(*out, img, (struct ms_rect){x, y, w, h});
}

static void blur(struct ms_image img, struct ms_image *out, char *argv[]) {
  int r = atoi(argv[0]);
  if (r <= 0) {
    fprintf(stderr, "Error: Invalid radius: %s\n", argv[0]);
    return;
  }
  *out = ms_alloc(img.w, img.h);
  ms_blur(*out, img, r);
}

static void threshold(struct ms_image img, struct ms_image *out, char *argv[]) {
  int t = strcmp(argv[0], "otsu") == 0 ? ms_otsu_threshold(img) : atoi(argv[0]);
  if (t <= 0) {
...
static void adaptive(struct ms_image img, struct ms_image *out, char *argv[]) {
  ...
  *out = ms_alloc(img.w, img.h);
  ms_adaptive_threshold(*out, img, r, c);
}

static void morph(struct ms_image img, struct ms_image *out, char *argv[]) {
  ...
  *out = ms_alloc(img.w, img.h);
  struct ms_image temp = ms_alloc(img.w, img.h);
  ms_copy(*out, img);
  for (int i = 0; i < n; i++) {
...
static void sobel(struct ms_image img, struct ms_image *out, char *argv[]) {
  (void)argv;
  *out = ms_alloc(img.w, img.h);
  ms_sobel(*out, img);
}

static void blobs(struct ms_image img, struct ms_image *out, char *argv[]) {
  int n = atoi(argv[0]);
  if (n <= 0) {
  ...
  *out = ms_alloc(img.w, img.h);
  ms_label *labels = (ms_label *)calloc(img.w * img.h, sizeof(ms_label));
  struct ms_blob *blobs = (struct ms_blob *)calloc(n, sizeof(struct ms_blob));
  if (!labels || !blobs) {
  ...
  unsigned nblobs = ms_blobs(img, labels, blobs, n);
  for (unsigned i = 0; i < nblobs; i++) {
    unsigned x1 = ms_MAX(0, (int)blobs[i].box.x - 2), y1 = ms_MAX(0, (int)blobs[i].box.y - 2);
    unsigned x2 = ms_MIN(img.w, blobs[i].box.x + blobs[i].box.w + 2),
             y2 = ms_MIN(img.h, blobs[i].box.y + blobs[i].box.h + 2);
    for (unsigned y = y1; y <= y2; y++) {
      for (unsigned x = x1; x <= x2; x++) { out->data[y * out->w + x] = 128; }
    }
  }
  ms_for(img, x, y) if (img.data[y * out->w + x] > 128) out->data[y * img.w + x] = 255;
}

static void draw_line(struct ms_image img, unsigned x1, unsigned y1, unsigned x2, unsigned y2,
                      uint8_t color) {
...
static void scan(struct ms_image img, struct ms_image *out, char *argv[]) {
  (void)argv;
  struct ms_image tmp = ms_alloc(img.w, img.h);
  // preprocess, remove noise, binarise
  ms_blur(tmp, img, 1);
  ms_threshold(tmp, ms_otsu_threshold(tmp) + 10);
  // find blobs
  ms_label *labels = calloc(img.w * img.h, sizeof(ms_label));
  struct ms_blob blobs[1000];
  unsigned n = ms_blobs(tmp, labels, blobs, sizeof(blobs) / sizeof(blobs[0]));
  // find largest blob
  unsigned largest = 0;
  for (unsigned i = 1; i < n; i++)
    if (blobs[i].area > blobs[largest].area) largest = i;
  // find corners
  struct ms_point corners[4];
  ms_blob_corners(tmp, labels, &blobs[largest], corners);
  // perspective correct
  const unsigned OUTPUT_WIDTH = 800, OUTPUT_HEIGHT = 1000;
  *out = ms_alloc(OUTPUT_WIDTH, OUTPUT_HEIGHT);
  ms_perspective_correct(*out, img, corners);

  ms_free(tmp);
  free(labels);
}
static int sort_keypoints(const void *a, const void *b) {
  const struct ms_keypoint *kp1 = (const struct ms_keypoint *)a,
                           *kp2 = (const struct ms_keypoint *)b;
  return kp2->response - kp1->response;
}

static void keypoints(struct ms_image img, struct ms_image *out, char *argv[]) {
  int n = atoi(argv[0]), t = atoi(argv[1]);
  if (n <= 0 || t < 0) {
    fprintf(stderr, "Error: Invalid number of keypoints or threshold\n");
    return;
  }
  struct ms_keypoint *kps = (struct ms_keypoint *)calloc(5000, sizeof(struct ms_keypoint));
  if (!kps) {
    fprintf(stderr, "Error: Memory allocation failed\n");
    return;
  }
  struct ms_image tmp = ms_alloc(img.w, img.h);
  unsigned nkps = ms_fast(img, tmp, kps, 5000, t);
  // sort keypoints by response (score)
  qsort(kps, nkps, sizeof(struct ms_keypoint), sort_keypoints);

  *out = ms_alloc(img.w, img.h);
  ms_copy(*out, img);
  for (unsigned i = 0; i < ms_MIN((unsigned)n, nkps); i++) {
    unsigned x = kps[i].pt.x, y = kps[i].pt.y, r = 2;
    for (int dy = -r; dy <= (int)r; dy++) ms_set(*out, x, y + dy, 255);
    for (int dx = -r; dx <= (int)r; dx++) ms_set(*out, x + dx, y, 255);
  }
  ms_free(tmp);
  free(kps);
}
...
static unsigned extract_pyramid_orb_nm(struct ms_image img, struct ms_keypoint *kps, unsigned nkps,
                                       unsigned threshold, uint8_t *buffer, unsigned n_levels) {
  if (n_levels > 4) n_levels = 4;
  struct ms_image pyramid[4];
  uint8_t *scoremaps[4];
  ...
  for (unsigned level = 1; level < n_levels; level++) {
    ...
    pyramid[level] = (struct ms_image){w, h, buffer + buffer_offset};
    buffer_offset += w * h;
    ms_downsample(pyramid[level], pyramid[level - 1]);
  }
  ...
  for (unsigned level = 0; level < n_levels; level++) {
    ...
    unsigned level_kps =
        ms_orb_extract(pyramid[level], &kps[total_kps], level_nkps, threshold, scoremaps[level]);
...
static void orb(struct ms_image img, struct ms_image *out, char *argv[]) {
  struct ms_image template = ms_read_pgm(argv[0]);
  if (!ms_valid(template)) {
    printf("Error: Cannot load template image %s\n", argv[0]);
    return;
  }

  static uint8_t buffer[1024 * 1024];
  static struct ms_keypoint template_kps[5000], scene_kps[5000];
  static struct ms_match matches[300];
  ...
  unsigned n_matches =
      ms_match_orb(template_kps, n_template, scene_kps, n_scene, matches, 300, 60.0f);
    ...
    for (unsigned i = 0; i < n_matches - 1; i++)
      for (unsigned j = i + 1; j < n_matches; j++)
        if (matches[j].distance < matches[i].distance) {
          struct ms_match temp = matches[i];
          matches[i] = matches[j];
          matches[j] = temp;
        }

    // Create stitched output
    *out = ms_alloc(template.w + img.w, ms_MAX(template.h, img.h));
    ...
    // Draw top matches
    unsigned draw_matches = ms_MIN(15, n_matches);
...
static void faces(struct ms_image img, struct ms_image *out, char *argv[]) {
  static uint32_t integral[640 * 480];  // Max typical image size
  static struct ms_rect rects[100];
  ...
  ms_integral(img, integral);
  unsigned nrects = ms_lbp_detect(&frontalface, integral, img.w, img.h, rects, 100, 1.2f, 1.0f,
                                  4.0f, min_neighbors);
...
struct cmd {
  ...
  int argc;
  int hasout;
  void (*func)(struct ms_image img, struct ms_image *out, char *argv[]);
} commands[] = {
...
int main(int argc, char *argv[]) {
  ...
  for (struct cmd *cmd = commands; cmd->name != NULL; cmd++) {
    ...
    struct ms_image img = ms_read_pgm(argv[cmd->argc + 2]);
    if (!ms_valid(img)) {
      fprintf(stderr, "Error: Could not load %s\n", argv[cmd->argc + 2]);
      return 1;
    }
    struct ms_image out = {0, 0, NULL};
    ...
    if (cmd->hasout) {
      ...
      if (ms_write_pgm(out, argv[cmd->argc + 3]) != 0) {
