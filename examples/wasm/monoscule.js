...
async function init() {
    try {
        ...
        let wasmModule;
        try {
            const { instance } = await WebAssembly.instantiateStreaming(fetch('monoscule.wasm'), importObject);
            wasmModule = instance;
        } catch (streamError) {
            console.warn("Streaming failed, trying fallback:", streamError);
            const response = await fetch('monoscule.wasm');
        ...
        alert("Failed to load monoscule.wasm. Make sure the file is present and the server is running correctly.");
        return;
    }
