...
testdata: nanomagick
	mkdir -p out
	./nanomagick identify testdata/monoscule.pgm
	./nanomagick view testdata/monoscule.pgm
...
nanomagick: examples/nanomagick/nanomagick.c monoscule.h
	$(CC) $(CFLAGS) -I. -o nanomagick examples/nanomagick/nanomagick.c $(LDFLAGS)

wasm: examples/wasm/monoscule.c monoscule.h
	clang \
		--target=wasm32 -O3 -flto -nostdlib -Wl,--no-entry -Wl,--export-all -Wl,--lto-O3 -DNDEBUG \
		-I. -o examples/wasm/monoscule.wasm examples/wasm/monoscule.c
